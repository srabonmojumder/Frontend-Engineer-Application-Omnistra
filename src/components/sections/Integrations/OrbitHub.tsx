...
export function OrbitHub({ integrations, isVisible }: OrbitHubProps) {
  ...
  return (
    <div
      className={cn(
        "relative mx-auto aspect-square w-full max-w-[420px] sm:max-w-[480px]",
        "transition-all duration-1000 ease-out",
        isVisible ? "opacity-100 scale-100" : "opacity-0 scale-90"
      )}
    >
      <div className="absolute inset-0 rounded-full border border-white/[0.03]" />
      <div className="absolute inset-[10%] rounded-full border border-dashed border-white/[0.05]" />
      ...
      <div className="absolute inset-[38%] flex items-center justify-center rounded-full bg-gradient-to-br from-[#3448FF] to-[#5B6AFF] shadow-[0_0_60px_rgba(52,72,255,0.3)]">
        <div className="flex flex-col items-center gap-1">
          ...
          <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-white/80">
            Hub
          </span>
        </div>
      </div>
      ...
      {innerRing.map((integration, i) => {
        ...
        return (
          ...
          >
            <div
              className="absolute h-px bg-gradient-to-r from-transparent via-white/[0.08] to-transparent"
              style={{
                width: `${radius * 0.7}%`,
            ...
            <div className="group relative animate-float" style={{ animationDelay: `${i * 1.5}s` }}>
              <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.06] backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.1] hover:scale-110 sm:h-14 sm:w-14">
        ...
        return (
          ...
          >
            <div className="group relative animate-float" style={{ animationDelay: `${i * 2 + 0.5}s` }}>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.1] hover:scale-110 sm:h-13 sm:w-13">
